...
for i in $(seq 1 30); do
    if docker compose exec -T clickhouse wget -qO- http://localhost:8123/ping 2>/dev/null | grep -q "Ok"; then
...
echo ""
echo "══════════════════════════════════════════════"
echo " Step 6: Waiting for Flow API…"
echo "══════════════════════════════════════════════"
for i in $(seq 1 20); do
    if curl -sf "http://localhost:8213/v1/peers/list" >/dev/null 2>&1; then
        echo "[setup_local] Flow API ready."
        break
    fi
    echo "  waiting… ($i/20)"
    sleep 3
...
echo "══════════════════════════════════════════════"
echo ""
echo "  PeerDB UI      → http://localhost:3100"
echo "  Flow API       → http://localhost:8213"
echo "  ClickHouse     → http://localhost:8144"
